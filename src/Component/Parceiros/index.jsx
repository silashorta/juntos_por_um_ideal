      ...
      {parceiros.map((parceiro, index) => (
        <div key={index} className='flex lg:flex-wrap gap-4 p-4 lg:justify-center overflow-y-auto'>
          {parceiro.imagem.map((imagem, i) => (
            <div key={i} className='lg:max-h-auto max-w-auto flex flex-wrap flex-col items-center gap-8 shadow-md  shadow-slate-500'>
              <div className='bg-gray-100 p-1 rounded h-44 w-40 lg:w-56 flex flex-col justify-center items-center gap-4'>
                <img src={`images/parceiros/${imagem}`} alt={`Foto ${parceiro.descricao[i]}`} className='max-h-16 lg:h-10 rounded' />
